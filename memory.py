...
def cs_toggle_triggerbot(toggle):
    if toggle == True:
        localplayer = cs_get_local_player()
        crosshairID = cs_get_crosshair_id()
        getTeam = pm.read_int(getdll() + dwEntityList + (crosshairID - 1) * 0x10)
        localTeam = cs_get_local_team()
        crosshairTeam = pm.read_int(getTeam + m_iTeamNum)

        if crosshairID > 0 and crosshairID < 64 and localTeam != crosshairTeam:
            wpm(getdll() + dwForceAttack, "int", 6)
            time.sleep(0.3)
